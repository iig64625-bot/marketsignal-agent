...
async def test_extract_events_returns_empty_for_short_text():
    ...
    with patch("marketsignal.events.extractor.get_llm") as get_llm_mock:
...
async def test_extract_events_returns_empty_when_no_api_key():
    ...
    with patch("marketsignal.events.extractor.get_llm", side_effect=ValueError("no key")):
...
async def test_extract_events_maps_llm_output_to_event_rows():
    ...
    from marketsignal.models.schemas import EventListOutput, EventOutput
    ...
    with patch("marketsignal.events.extractor.get_llm", return_value=fake_llm):
        out = await extract_events(_make_doc())
    assert len(out) == 1
    ev = out[0]
