...
```bash
# CLI 模式
signalpulse run --config configs/competitors.ai-agent.yaml
...
```bash
signalpulse run --use-sample-dataset
```
...
- [x] **8-tab Streamlit UI**（仪表盘 / 竞品总览 / 周报 / 战报 / 对比 / 评估 / 运行 / 调度 / 设置）
- [x] **新用户 onboarding 引导**（紫色卡片 + 一键加载示例数据）
- [x] **4 联筛选**（搜索 + 时间范围 + 信号类型 + 竞品）
- [x] **评估指标趋势图**（多线跨 8 次运行）
- [x] **报告分享功能**（独立 HTML 快照 + base64 下载）
- [x] **竞品总览**（每竞品一张卡片：信号数 + 最新动态 + 类型分布）
- [x] **39+ 单元测试**（diff / export / notify / scheduling / share / overview）
