...
> 本文用于面试现场的 5 分钟现场演示 + Q&A 准备。所有命令都在 PowerShell + venv 环境下验证过（200+ 测试全绿）。
...
> 三个改动：
> 1. `DATABASE_URL` 指向 Postgres（已经走 SQLAlchemy，0 代码改动）
...
> "SignalPulse 是给产品/销售团队用的 AI 竞品情报系统。它主动抓竞品公开数据，过 LangGraph 多 Agent 工作流（不是单个 prompt），生成**带引用证据**的周报和销售 Battlecard，并自动跑 5 个 eval 指标（引用覆盖率、未支撑率、去重率、延迟、token 成本）。整套系统在本地 SQLite 上跑，200+ 测试 1 秒内绿，样例数据 0 网络可演示。"
