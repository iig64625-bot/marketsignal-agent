...
### 通知系统 (`signalpulse.notify`)

`Notification` 抽象基类 + 三个实现：

| 实现 | 用途 | 关键细节 |
|---|---|---|
| `FeishuWebhook` | 飞书机器人卡片 | HMAC-SHA256 签名（**base64** 不是 hex）；失败重试 |
| `EmailSender` | SMTP 邮件 | TLS + login；multipart/alternative |
| (factory) `build_notifier()` | 优先级 feishu > email | 没配返回 `None` |

设置走 `.env`：`FEISHU_WEBHOOK_URL` / `FEISHU_WEBHOOK_SECRET` / `SMTP_*` / `NOTIFY_*`。

### 后台调度 (`signalpulse.scheduler`)

APScheduler `BackgroundScheduler` + cron 触发；状态持久化到 `data/scheduler/jobs.json`。
CLI 入口：`python -m signalpulse.cli schedule --cron "0 9 * * 1" --name weekly`。
Daemon 入口：`python scripts/scheduler_daemon.py`（从 `schedules.yaml` 读所有 job）。

### WebSocket 实时进度 (`api/routes/ws.py`)

`/ws/runs/{run_id}` 推 `snapshot` / `node` / `status` / `heartbeat` / `done` 消息。
Streamlit UI `components/run_monitor.py` 渲染实时面板 + 断线重连。

### 周对比 (`reporting/diff.py`)

`compute_diff(run_a, run_b) -> RunDiff`：信号差 / 声明差 / 每竞品差 / 新增 + 消失的 claims。

### 报告导出 (`reporting/export.py`)

`export_to_pdf` (fpdf2) + `export_to_excel` (openpyxl)；可选依赖 `pip install ".[pdf]"`。

---

## V5 增量（2026-06）

### 8-tab Streamlit UI (`src/signalpulse/ui/`)

| Tab | 文件 | 用途 |
|---|---|---|
| 📊 仪表盘 | `components/dashboard.py` | 4 metric 卡 + 评估指标趋势图 + 4 联筛选（搜索/时间/类型/竞品） |
| 🏢 竞品总览 | `components/competitor_overview.py` | 每竞品一张卡片：信号数 + 最新动态 + 类型饼图 |
| 📋 周报 / ⚔️ 战报 | `components/report_viewer.py` | Markdown 报告列表 + 暗色背景 |
| 🔄 对比分析 | `components/comparison.py` | 选两次运行 diff（带 baseline/current label） |
| 📈 评估指标 | `render_signal_timeline` | 8 次运行趋势 + 4 核心指标 |
| 📝 运行记录 | `components/run_monitor.py` | 实时 WebSocket + 断线 "⚠️ auto-retry in 2s" |
| ⏰ 定时任务 | `components/scheduling.py` | Cron 输入 + 写 `schedules.yaml` |
| ⚙️ 设置 | `components/settings.py` + `notification_config.py` | 飞书/钉钉 webhook + 测试发送 + 写 `.env` |

**Onboarding**：空状态显示紫色卡片 + "📥 一键加载示例数据" 按钮，零网络零 token 跑通。

**i18n** (`ui/i18n.py`)：zh / en 双语，`t(key)` 函数 + `render_lang_toggle()` 切换。

**报告分享** (`components/share.py`)：把任意 Markdown 报告渲染成独立暗色 HTML 快照，base64 下载。

### 39+ 单元测试

| 文件 | 测试目标 | 测试数 |
|---|---|---|
| `tests/reporting/test_diff.py` | compute_diff + 内部 helper | 9 |
| `tests/reporting/test_export.py` | _strip_md + export_to_pdf/excel | 10 |
| `tests/notify/test_notify.py` | feishu 签名 + 发送 + email + factory | 10 |
| `tests/ui/test_scheduling.py` | _load/_save/_next_run + cron | 7 |
| `tests/ui/test_share.py` | _render_html 5 个属性 | 6 |
| `tests/ui/test_competitor_overview.py` | _companies_with_stats + source 静态检查 | 4 |
| `tests/test_scheduler.py` | scheduler.add_job/list_jobs/remove_job | 6 |

### Bug 修复

- `competitor_overview.py` 列表推导 `t or "unknown"` 覆盖了 i18n `t` → 改为 `sig_type or "unknown"`
- `pyproject.toml` 加 `openpyxl` / `plotly` / `croniter` 依赖（之前只 `pdf` optional）
- `competitor_overview` plotly 缺时降级 `st.bar_chart`

### 一键脚本

| 脚本 | 用途 |
|---|---|
| `run.ps1` | 前台跑 streamlit |
| `run_scheduled.ps1` | 后台跑 streamlit + scheduler daemon（从 `schedules.yaml` 读 jobs） |
| `stop_all.ps1` | 杀 streamlit + scheduler |
| `scripts/scheduler_daemon.py` | APScheduler 后台守护，读 `schedules.yaml` |
...
```bash
docker build -t signalpulse-agent .
docker run -p 8000:8000 -p 8501:8501 signalpulse-agent
```

Dockerfile 现在用 2-stage build（builder + runtime），包名 `signalpulse`（不是 `marketsignal`），暴露 8000 + 8501。