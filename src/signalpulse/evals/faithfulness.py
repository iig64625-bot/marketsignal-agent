...
def llm_judge_score(summary: str, source: str) -> float | None:
    ...
    try:
        ...
        return None
    # Extra guard: get_llm() may succeed (e.g. provider=ollama) but the
    # model cannot actually score faithfulness without a real API. If no
    # paid-provider key is set, bail out early.
    ...
    if not any(_paid_keys):
        logger.info("faithfulness: no paid-provider API key set, skipping LLM judge")
        return None
