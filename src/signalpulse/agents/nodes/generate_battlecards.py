"""Node: render per-competitor battlecards from signals."""
...
from signalpulse.reporting.battlecard_extras import generate_battlecard_extras
from signalpulse.reporting.render_markdown import render_battlecard
from signalpulse.utils.tracing import trace_node


@trace_node("generate_battlecards_node")
async def generate_battlecards_node(state: GraphState) -> GraphState:
    run_id = state["run_id"]
    ...
    with get_session() as s:
        ...
        for company_id, sigs in groups.items():
            ...
            try:
                ...
                markdown = render_battlecard(sigs, company_name=company.name, target_name=target, extras=extras)
            except (ValueError, TypeError):
                markdown = f"# Battlecard: {company.name}\n\n_(render error)_"
            path = out_dir / f"battlecard_{company.name}_{run_id}.md"
            ...
            report = Report(
                ...
                title=f"Battlecard: {company.name}",
                markdown_path=str(path),
                json_path=None,
            )
            s.add(report)
            new_ids.append(report.id)
    report_ids = list(state.get("report_ids", [])) + new_ids
