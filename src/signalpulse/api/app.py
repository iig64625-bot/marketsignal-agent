"""FastAPI application factory for the SignalPulse Agent API.

Use ``uvicorn signalpulse.api.app:create_app --factory`` to run.
"""
...
from signalpulse.api.routes import (
    health_router,
    metrics_router,
    reports_router,
    runs_router,
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info(
        "SignalPulse API starting: env={} db={} llm_provider={}",
        settings.app_env,
    ...
    yield
    logger.info("SignalPulse API shutting down")
...
def create_app() -> FastAPI:
    ...
    settings = get_settings()
    app = FastAPI(
        title="SignalPulse Agent API",
        version="0.1.0",
        description="REST API for the SignalPulse competitive-intelligence agent.",
        lifespan=lifespan,
    )
    app.add_middleware(
        CORSMiddleware,
