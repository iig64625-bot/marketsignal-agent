"""CLI entry point for the SignalPulse Agent pipeline."""
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="signalpulse")
    ...
    schedule_p.add_argument("--name", default="signalpulse-job", help="Job name (also used as ID)")
...
def _run_pipeline(config_path: str, *, use_sample_dataset: bool, target_override: str | None = None) -> int:
    ...
    """
        # Auto-init DB tables in dev mode (skips if already created).
    try:
        import signalpulse.models  # noqa: F401  (registers all models)
        from signalpulse.db.engine import get_engine
        from signalpulse.models.base import Base
        Base.metadata.create_all(get_engine())
    except Exception as exc:
        logger.debug("cli: DB auto-init skipped: {}", exc)
...
def _run_check_sources(config_path: str) -> int:
    ...
    from signalpulse.ingestion.health_check import check_all_sources, summarize
...
def _run_schedule_command(args) -> int:
    ...
    from signalpulse.scheduler import add_job, list_jobs, shutdown
    console = Console()
    add_job(
        name=args.name,
        cron=args.cron,
...
def _run_rag_eval(args) -> int:
    ...
    from signalpulse.evals.rag_eval import evaluate, load_gold_set
